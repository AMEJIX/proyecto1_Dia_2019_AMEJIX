...
let mostrarUtiles = (pfiltro) =>{
    ...
    let contador = 0;
        let utiles = listarUtilesNivelPropio(nivel, IdUs, pfiltro);        
        tabla.innerHTML = '';        
...
let obtenerDatosArticulo = (p_id, pidCampoCantidad, pvalorCantidad) =>{
    if(validarCampoCantidad(pidCampoCantidad, pvalorCantidad) == false){
        ...
        Swal.fire({
            ...
            if (result.value) {
                actualizarUtilCE(cantidad, p_id, nombreUsuario);
            }
          })
    }else{
        swal.fire({
...
// inputFiltro.addEventListener('keyup', funcionPrueba3 =>{    
//     mostrarUtiles(parametro);
// });
